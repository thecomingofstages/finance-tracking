...
export const DepartmentSection: React.FC<DepartmentSectionProps> = ({ projectId, departments, isPrivileged, onRefresh }) => {
  ...
  const [editingDept, setEditingDept] = useState<ProjectDepartment | null>(null);
  ...
  const handleDelete = async (deptId: string) => {
    if (!confirm("ต้องการลบฝ่ายนี้หรือไม่?")) return;
    setIsDeleting(deptId);
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formName.trim()) { setError("กรุณากรอกชื่อฝ่าย"); return; }
    ...
    <>
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <h2 className="font-semibold text-slate-800">ฝ่าย (Departments)</h2>
          {isPrivileged && (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200/80 text-xs font-semibold text-slate-600 uppercase tracking-wider">
                <th className="py-3 px-5">ชื่อฝ่าย</th>
                <th className="py-3 px-5 text-right">กำหนดงบไว้</th>
                <th className="py-3 px-5 text-right">ใช้จริง</th>
                <th className="py-3 px-5 w-48">สัดส่วน</th>
                {isPrivileged && <th className="py-3 px-5 w-20"></th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
              {departments.length === 0 ? (
                <tr><td colSpan={isPrivileged ? 5 : 4} className="py-8 text-center text-slate-400 text-sm">ยังไม่มีฝ่าย</td></tr>
              ) : (
                departments.map((dept) => {
                  const deptId = dept._id || dept.id;
                  const budget = Number(dept.allocated_budget) || 0;
                  const expense = Number(dept.total_expense) || Number(dept.actual_expense) || 0;
                  const pct = budget > 0 ? Math.min(Math.round((expense / budget) * 100), 100) : 0;
                  const isOver = expense > budget && budget > 0;
                  return (
                      ...
                      <td className="py-3 px-5">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all ${isOver ? "bg-rose-500" : pct > 80 ? "bg-amber-500" : "bg-blue-600"}`}
                      ...
                      {isPrivileged && (
                        <td className="py-3 px-5">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                            <button onClick={() => openEditModal(dept)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                            </button>
                            <button disabled={isDeleting === deptId} onClick={() => deptId && handleDelete(deptId)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50">
                              {isDeleting === deptId ? (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
              <h3 className="font-semibold text-slate-800">{editingDept ? "แก้ไขฝ่าย" : "เพิ่มฝ่าย"}</h3>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {error && <div className="bg-red-50 text-red-600 text-sm p-3 rounded-xl border border-red-100">{error}</div>}
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-700">ชื่อฝ่าย <span className="text-red-500">*</span></label>
              ...
              <div className="pt-2 flex gap-3">
                ...
                <button type="submit" disabled={isSubmitting} className="flex-1 px-4 py-2.5 bg-blue-900 text-white hover:bg-blue-800 rounded-xl font-medium transition-all disabled:opacity-50">
                  {isSubmitting ? "กำลังบันทึก..." : (editingDept ? "บันทึก" : "เพิ่ม")}
                </button>
              </div>
            </form>
          </div>
