...
export const SignatureUploadModal: React.FC<SignatureUploadModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-slate-900 rounded-2xl shadow-2xl border border-slate-800 overflow-hidden text-slate-100">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} noValidate className="p-5 space-y-4">
          ...
          <div>
            ...
            {previewUrl ? (
              <div className="relative border border-slate-800 rounded-xl p-4 flex flex-col items-center justify-center bg-slate-950/60">
                {/* Image Preview */}
                <img
                  src={previewUrl}
                  alt="Signature preview"
                  className="max-h-28 object-contain mb-2 filter invert brightness-200"
                />
              ...
              <div
                ...
                className="border-2 border-dashed border-slate-800 hover:border-slate-600 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer bg-slate-950/50 transition-colors"
              >
                <svg
                ...
                <p className="text-[11px] text-slate-500 mt-1">รองรับเฉพาะ PNG และ JPEG</p>
              </div>
            )}
          ...
          <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-800">
            <button
              ...
              onClick={onClose}
              disabled={isLoading}
              className="px-3.5 py-2 text-xs font-medium text-slate-400 hover:text-slate-200 transition-colors disabled:opacity-50"
            >
              ไว้ทีหลัง
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="px-4 py-2 rounded-xl text-xs font-medium text-slate-950 bg-slate-100 hover:bg-white transition-all shadow disabled:opacity-50 flex items-center gap-2 cursor-pointer"
            >
              {isLoading ? (
                <>
                  <svg
                    className="animate-spin h-3.5 w-3.5 text-slate-900"
